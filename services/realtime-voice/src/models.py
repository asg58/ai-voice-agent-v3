"""
Data models for Real-time Conversational AI
"""
...
class ConversationMessageDetail(BaseModel):
    """Detailed conversation message with additional metadata"""
    session_id: str = Field(..., description="Session ID")
...
class ConversationAnalysisResult(BaseModel):
    ...
    topics: Dict[str, float] = Field(..., description="Topic detection")
    intents: Dict[str, int] = Field(..., description="Intent recognition")
    entities: Dict[str, List[str]] = Field(..., description="Entity extraction")


class EmotionRecognitionResult(BaseModel):
    """Emotion recognition result"""
    session_id: str = Field(..., description="Session ID")
    emotion: str = Field(..., description="Detected emotion")
    score: float = Field(..., description="Confidence score")
    all_scores: Dict[str, float] = Field(..., description="All emotion scores")
    audio_emotion: Optional[str] = Field(None, description="Audio-based emotion")
    text_emotion: Optional[str] = Field(None, description="Text-based emotion")
...
class TranslationResult(BaseModel):
    ...
    timestamp: float = Field(..., description="Translation timestamp")
...
class AudioDataMessage(WebSocketMessage):
    """Audio data WebSocket message"""
    type: str = Field("audio_data", description="Message type")
...
class TranscriptionMessage(WebSocketMessage):
    """Transcription result WebSocket message"""
...
class TTSMessage(WebSocketMessage):
    """TTS audio WebSocket message"""
    type: str = Field("tts_audio", description="Message type")
    audio_data: str = Field(..., description="Base64 encoded audio data")
    text: str = Field(..., description="Original text")
